...
- Pregunta recurrente: "¿Las allowlists viajan con el repo?" — `.claude/settings.local.json` está pensado para no commitearse (entra en `.gitignore` típicamente, son overrides personales). `.claude/settings.json` sí se commitea y es lo que viaja con el repo. En este tema plantamos `.claude/settings.json` porque es lo coherente para gobierno de equipo (allowlists revisables en PR).
