# Tema 27 — Proyecto final: flujo completo de ingeniería sobre un repositorio real

> **Tipo:** proyecto final guiado (capstone) **Duración estimada:** sesión completa de proyecto
...
Abre Claude a las herramientas reales del equipo (Jira, CI, base de datos, observabilidad) **solo donde aporta valor de verdad** — no por coleccionar conectores. Para cada integración candidata, responde en una línea: _¿qué tarea concreta del proyecto desbloquea?_ Si no sabes responder, no la conectes.
...
> Dejar que la IA genere commits sin juicio produce un historial ruidoso que no ayuda a nadie. El PR explica el _por qué_, no solo el _qué_.
...
> **Esto es una guía genérica.** No es _el_ método correcto: es un esqueleto probado que adaptas a tu forma de trabajar, a tu equipo y a tus herramientas. Donde aquí pone "Obsidian", lee "Confluence, Notion, Jira, un `.md` en el repo o un papel".
...
* Te obligas a pensar el _qué_ y el _por qué_ antes que el _cómo_. Las decisiones de diseño las tomas tú, no el modelo.
...
* _Cuándo:_ tienes lógica sin cubrir, o quieres blindar un bug recién arreglado para que no vuelva.
* _Qué documentas:_ qué comportamiento **crítico** debe quedar cubierto — no "subir el % de coverage".
*   _Prompt clave:_
...
* _Cuándo:_ te toca aprobar un cambio, o quieres auto-revisarte antes de pedir review.
* _Qué documentas:_ qué te preocupa de este cambio (el área de riesgo que más miras).
*   _Prompt clave:_
...
* _Cuándo:_ bump con breaking changes (p. ej. React 18→19, un framework, una librería core).
* _Qué documentas:_ versión origen/destino y qué partes del código usan la API que cambia.
*   _Prompt clave:_
...
* _Cuándo:_ un merge o rebase con conflictos que no son triviales.
* _Qué documentas:_ qué hacía **cada rama** en la zona en conflicto (la intención de ambos lados).
*   _Prompt clave:_
