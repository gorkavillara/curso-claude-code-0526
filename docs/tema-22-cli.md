...
# Tema 22 — Opciones avanzadas de CLI, sesiones, historiales y productividad intensiva en terminal
...
> `compact` **no es magia**: es un resumen. Después de compactar, el agente sabe *menos* detalles concretos. Si la tarea exige fidelidad fina (debug de un error oscuro), valora si te conviene cerrar y abrir limpio en vez de compactar.
